        ...
        $(".copy-lead__button").on("click", function () {
          if (!$(this).hasClass("copy-lead__button_disable")) {
            let pipeline_id = Number(
              ...
              info_block = $(".copy-lead__info");

            info_block
              .text("")
              .removeClass(
                "copy-lead__info_load copy-lead__info_error copy-lead__info_success"
              );
            ...
            $.ajax({
              ...
              beforeSend: function () {
                info_block
                  .text("Загрузка сделки...")
                  .addClass("copy-lead__info_load");
              },
              error: function () {
                info_block
                  .text("Ошибка загрузки сделки")
                  .addClass("copy-lead__info_error");
              },
              success: function (lead) {
                info_block
                  .text("Сделка загружена")
                  .addClass("copy-lead__info_load");
                ...
                $.ajax({
                  ...
                  error: function () {
                    info_block
                      .text("Ошибка загрузки контактов")
                      .addClass("copy-lead__info_error");
                  },
                  success: function (data) {
                    info_block
                      .text("Контакты загружены")
                      .addClass("copy-lead__info_load");

                    let contacts_id = [];
                    data._embedded.links.forEach((elem) => {
                      if (elem.to_entity_id != null && elem.to_entity_id != "")
                        contacts_id.push(elem.to_entity_id);
                    });
                    ...
                    $.ajax({
                      ...
                      error: function () {
                        info_block
                          .text("Ошибка создания новой сделки")
                          .addClass("copy-lead__info_error");
